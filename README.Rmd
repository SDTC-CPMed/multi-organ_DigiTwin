...
filename = "data/UMI_expression_matrix.txt.gz"
```
...
We performed these analyses for all the organ combined. 
...
sc_data_quality_sorting(filename)
```
...
```{r}
source("Python/ConvertReferencesSandra.ipynb")
```
...
```{bash}
source activate /home/sanli71/.conda/envs/scvi

# DEG analysis, sick vs healthy
infile=../DGE_data/sorted_DGEs/allTissues_sorted_expression_matrix_trs.csv
outdir=../results/allTissues_tissue-sample-BatchRemoval/
clust_file=../results/clusters_final_out/cluster_ids_fromOleg_06_29.csv
#outdir_DEG=../results/DEG_analysis/cluster_ids_fromOleg_06_29_CellType/
outdir_DEG=../results/DEG_analysis/cluster_ids_fromOleg_06_29_CellType/no_CellTyping
mkdir -p $outdir_DEG
mkdir -p $outdir_DEG/vanilla_mode
mkdir -p $outdir_DEG/change_mode
python scVI_v0.7.1_test.py --infile $infile --outdir $outdir --n_epochs 400 --DEG_analysis --clust_file $clust_file --outdir_DEG $outdir_DEG --sort_column 4 --group_column 3 --use_batches --batch_id 0 2
wait
