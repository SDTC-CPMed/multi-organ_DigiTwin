...
We further subset these tables only to include significant DEGs, and columns relevant for downstream analyses. The output from this analysis consists of one table of significant DEGs for each input table of DEGs, which are saved to '[outdir_DEG]/subset_significant' (Table 2).
...
The significant DEGs were then identified based on fdr corrected p-velues, as described [above](#data-normalization-and-differential-expression-analysis).
...
![Figure 2. Heatmap indicating the significantly upregulated marker genes (1) vs not significantly upregulated marker genes (0) in the different cell types/clusters.](./data/clustering_and_celltype_identification/heatmap_markerGenes.png){width=65%}

Based on Figure 1 and Figure 2, we verified the [previous cell type identification](#clustering-and-cell-type-identification-by-leidens-algorithm), and further identified the unknown cell types as Myocytes (Unknown0), Neutrophils (Unknown16), and Basal III cells (Unknown6) respectively. We update the file containing the clusters information to include these cell type identities. 
...
* the cell type identification file. Output from [cell type analysis script](#clustering-and-cell-type-identification-by-leidens-algorithm)
